...
> 不要根据本简报的任何内容自行调整孩子的用药、剂量或复诊安排。任何治疗改变都应由主诊医师结合完整病史决定。  
...
> 本节按 `references/caregiver-layer.md` 撰写。绝大多数月份第 1 句就是“本月没有需要改变任何事的证据”，这是正常且合格的结论。

1. **本月结论**：{{本月没有需要改变任何事的证据 / 本月有 {{n}} 项值得在门诊提出的证据}}
2. **与你孩子的关系**：{{直接相关 / 间接相关 / 目前不相关，一句话说明原因；无 patient-profile 时写“未配置患儿画像，无法判断个体相关性”}}
3. **你需要做的事**：{{无需行动 / 下次门诊带上第「门诊问题」节的问题 / 建议主动联系医疗团队（仅限重要安全信号）}}

## 下次门诊可以问医生的问题

> 最多 3 条。必须具体、附出处、以“这对我们适用吗”的形式提出。
> 禁止出现换药/加药/停药/减量/剂量/化验解读等内容。本期无可问内容时写“本期无”。

1. **{{问题原文}}**
   - 出处：{{PMID / DOI / 注册号}}
   - 为什么问这个：{{对应哪条证据，医生的回答会如何影响你的理解——不是影响用药}}

## 与当前用药相关的安全信息

> 仅在配置了 patient-profile 且本期检出与患儿**当前实际用药**相关的安全性证据时填写；否则写“本期无”。
> 已在说明书和医嘱中交代过的常见不良反应，不重复报告成新发现。

- {{信号}} · 证据强度 S{{1-5}} · 人群直接性 {{direct/indirect/mixed}} · {{新信号 / 既有已知风险的再确认}}
- **这不构成停药或调整剂量的理由。**{{需要时：可作为门诊问题提出。}}
